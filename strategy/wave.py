...
from brokers_new import BrokerGateway, OrderRequest, Exchange, OrderType, TransactionType, ProductType
import datetime
import time
import yaml
import logging
...
class WaveStrategy:
    ...
    def _execute_orders(
        self, symbol: str, final_buy_price: float, final_sell_price: float,
        restrict_buy_order: int, restrict_sell_order: int
    ) -> None:
        """Execute buy and sell orders based on restrictions"""
        sell_order_id = None
        logger.info(
            f"Executing orders for {symbol} | Restrictions - Buy: {restrict_buy_order}, Sell: {restrict_sell_order}"
        )
        ...
        if restrict_sell_order == 0:
            try:
                ...
                logger.info(f"Sell Order Response - {sell_order_resp}")

                sell_order_id = getattr(sell_order_resp, "order_id", None)
                if not sell_order_id:
                    logger.error("Sell order failed — broker returned no order_id.")
                    return
                ...
                if (sell_order_id in self.handle_order_update_call_tracker_response_dict
                    and not self.handle_order_update_call_tracker[sell_order_id]):
                    self.handle_order_update(
                        self.handle_order_update_call_tracker_response_dict[sell_order_id]
                    )
                else:
                    logger.warning(f"No tracker response dict found for sell order {sell_order_id}")

            except Exception as e:
                logger.exception(f"Error placing SELL order for {symbol}: {e}")
                return

        # === BUY ORDER BLOCK ===
        if (restrict_sell_order == 1 or sell_order_id) and restrict_buy_order == 0:
            try:
                req = OrderRequest(
                ...
                logger.info(f"Buy Order Response - {buy_order_resp}")

                buy_order_id = getattr(buy_order_resp, "order_id", None)
                if not buy_order_id:
                    logger.error("Buy order failed — broker returned no order_id.")
                    if sell_order_id:
                        logger.warning(f"Buy order failed, cancelling associated sell order {sell_order_id}")
                        self._remove_order(sell_order_id)
                        self.handle_order_update_call_tracker.pop(sell_order_id, None)
                    return
                ...
                if sell_order_id and sell_order_id in self.orders:
                ...
                if (buy_order_id in self.handle_order_update_call_tracker_response_dict
                    and not self.handle_order_update_call_tracker[buy_order_id]):
                    self.handle_order_update(
                        self.handle_order_update_call_tracker_response_dict[buy_order_id]
                    )
                else:
                    logger.warning(f"No tracker response dict found for buy order {buy_order_id}")

            except Exception as e:
                logger.exception(f"Error placing BUY order for {symbol}: {e}")
...
if __name__ == "__main__":
    ...
    from orders_new import OrderTracker
...
• Margin Parameters: margin-spread, margin-single-pe-ce, margin-both-pe-ce
    ...
    def validate_configuration(config):
        ...
        default_values = {
            'symbol_name': 'NIFTY25OCTFUT',  # This is a placeholder value
            'buy_gap': 25,
            'sell_gap': 25,
            'buy_quantity': 75,
            'sell_quantity': 75,
